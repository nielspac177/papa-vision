...
testing and calibration analysis. Strikingly, the from-scratch custom CNN attains
...
saliency maps nonetheless reveal that all models partly attend to image background
rather than leaf lesions, exposing the well-documented background bias of
PlantVillage and tempering the optimistic accuracies. We release a fully
...
ahead of ResNet-18 (Fig.~\ref{fig:comparison}). Paired McNemar tests between the
custom CNN and each frozen-backbone transfer model are all significant
($p=\mcnemarbestp$ versus \besttransfer{}, and smaller against the others; see the
repository's \texttt{summary.json}), so the advantage is statistically reliable on
this shared test set and not an artefact of a single seed. The interpretation is
...
Fig.~\ref{fig:reliability} shows reliability diagrams and ECE values. All models are
\emph{moderately overconfident}: the custom CNN's ECE is $\eceCustom$, and the
transfer models range similarly (roughly $0.13$--$0.17$). The bars sit below the
diagonal at high confidence, the classic signature of overconfidence reported for
modern networks. This is an honest limitation: while the custom CNN is both the most
accurate and the best calibrated of the four, none is calibrated well enough to take
its raw confidences at face value, and a deployed system would benefit from
post-hoc calibration (e.g.\ temperature scaling) before using confidence to defer
...
budget}. Full fine-tuning of the backbones would very likely raise their accuracy
...
classification on commodity hardware. The compact custom model---two orders of
magnitude smaller than ResNet-18---significantly outperforms the frozen transfer
baselines under an equal low-compute budget, and Grad-CAM reveals a shared reliance
