...
% Load the auto-generated metric macros FIRST (real \newcommand definitions);
% the \providecommand fallbacks below then only fill in anything still missing,
% so the document also compiles standalone before metrics are generated.
...
\providecommand{\bestmodel}{--}\providecommand{\bestfone}{--}\providecommand{\besttransfer}{the best transfer model}\providecommand{\besttransferfone}{--}
...
MobileNetV2, ResNet-18 and EfficientNet-B0 with frozen ImageNet backbones and
fine-tuned heads, evaluating every model over
$\nseeds$ random seeds, reporting across-seed standard deviations and non-parametric
bootstrap confidence intervals, with McNemar significance testing and calibration
analysis. Strikingly, the from-scratch custom CNN attains
$\foneCustom\%$ macro-F1, \emph{exceeding} the strongest transfer baseline
(\besttransfer{}, $\besttransferfone\%$); paired McNemar tests confirm that it
significantly outperforms all three frozen-backbone transfer models ($p<0.05$)
while using roughly $\paramratiomin$--$\paramratiomax\times$ fewer parameters. The
result indicates that, under an equal low-compute budget, learning compact
domain-specific features end-to-end beats reusing frozen ImageNet features. Grad-CAM
saliency maps nonetheless indicate that all models partly attend to image background
rather than leaf lesions, qualitatively consistent with the well-documented
background bias of PlantVillage and tempering the optimistic accuracies. We release a fully
...
\begin{enumerate}
  ...
  CPU or Apple-Silicon hardware, benchmarked head-to-head against transfer-learning
  baselines (MobileNetV2, ResNet-18 and EfficientNet-B0 with frozen ImageNet
  backbones and fine-tuned classification heads).
  \item A statistically rigorous evaluation---multi-seed runs with across-seed
  standard deviations, per-run bootstrap 95\% confidence intervals, and McNemar's
  paired test on a shared test set~\cite{dietterich1998mcnemar}---rather than a
...
\begin{figure}[t]
\centering
\IfFileExists{figures/dataset_samples.png}{\includegraphics[width=0.85\linewidth]{dataset_samples.png}}{\fbox{\parbox{0.6\linewidth}{\centering\vspace{1cm}[dataset\_samples.png]\vspace{1cm}}}}
...
each as a mean and standard deviation over $\nseeds$ random seeds (Table~\ref{tab:results},
Fig.~\ref{fig:comparison}). To quantify within-test-set sampling uncertainty we
additionally compute non-parametric 95\% confidence intervals by bootstrap resampling
of the test set for every run; these are released with the per-run metrics and the
best model's intervals are quoted in the Results. We compare models pairwise on their
shared test set with McNemar's test~\cite{dietterich1998mcnemar}, using the exact
binomial form when the number of discordant predictions is small. We assess
...
\section{Results}
\IfFileExists{tables.tex}{\input{tables}}{}
...
laboratory-controlled nature of PlantVillage. The headline result, however, is that
the from-scratch custom CNN is not merely competitive but the \emph{best} performer:
with only $\paramsCustom$ parameters it attains $\foneCustom\%$ macro-F1, ahead of
the strongest transfer baseline (\besttransfer{}, $\besttransferfone\%$) and well
ahead of ResNet-18 (Fig.~\ref{fig:comparison}). Paired McNemar tests on the shared
(seed-0) test set find the custom CNN significantly better than each frozen-backbone
baseline ($p=\mcnemarbestp$ versus \besttransfer{}, and smaller against the others;
see the repository's \texttt{summary.json}). A non-parametric bootstrap on that test
set places the custom CNN's macro-F1 at $[\bestfoneCIlo, \bestfoneCIhi]\%$ (95\% CI)
and its accuracy at $[\bestaccCIlo, \bestaccCIhi]\%$. The significance test is
computed on a single seed's predictions, but the ranking is not seed-specific: across
all three seeds the custom CNN's mean macro-F1 ($\foneCustom\%$) exceeds every
baseline mean. The interpretation is
that, when only a small classification head can be trained on top of a frozen
ImageNet backbone, those backbones contribute generic features poorly matched to
leaf pathology, whereas the compact CNN---trained end-to-end---learns features
specific to the task. Validation macro-F1 curves (Fig.~\ref{fig:curves}) show all
models reaching a high plateau within the training budget.

\begin{figure}[t]
\centering
\IfFileExists{figures/model_comparison.png}{\includegraphics[width=0.7\linewidth]{model_comparison.png}}{\fbox{\parbox{0.6\linewidth}{\centering\vspace{1cm}[model\_comparison.png]\vspace{1cm}}}}
...
\begin{figure}[t]
\centering
\IfFileExists{figures/training_curves.png}{\includegraphics[width=0.62\linewidth]{training_curves.png}}{\fbox{\parbox{0.6\linewidth}{\centering\vspace{1cm}[training\_curves.png]\vspace{1cm}}}}
...
The confusion matrices (Fig.~\ref{fig:confusion}) are informative beyond the headline
accuracy. For the best model (custom CNN) the few errors are predominantly
disease-versus-disease confusions---an operationally benign pattern, since both
diseased states warrant intervention. The weaker frozen-transfer models, by contrast,
additionally misclassify a non-trivial number of \emph{late-blight} leaves as
\emph{healthy} (for example, 10--12 of 150 for MobileNetV2 and ResNet-18): a far more
concerning false-negative mode, because it would tell a farmer an infected plant is
fine. This asymmetry---the from-scratch model not only scoring higher but failing
more safely---reinforces the case for the compact end-to-end model. The minority
healthy class is recognised reliably across models, indicating that class weighting
counteracts the imbalance.

\begin{figure}[t]
\centering
\IfFileExists{figures/confusion_matrices.png}{\includegraphics[width=0.98\linewidth]{confusion_matrices.png}}{\fbox{\parbox{0.6\linewidth}{\centering\vspace{1cm}[confusion\_matrices.png]\vspace{1cm}}}}
...
Fig.~\ref{fig:reliability} shows reliability diagrams and ECE values (for a
representative seed; Table~\ref{tab:results} reports the seed-averaged ECE). All
models are imperfectly calibrated, with seed-averaged ECE ranging from $\eceCustom$
(custom CNN, the best calibrated) to about $0.17$ (ResNet-18). The diagrams indicate
that the miscalibration is driven mainly by mid-confidence bins, where empirical
accuracy and predicted confidence diverge rather than by uniform over-confidence.
This is an honest limitation: while the custom CNN is both the most accurate and the
best calibrated of the four, none is calibrated well enough to take its raw
confidences at face value, and a deployed system would benefit from post-hoc
calibration (e.g.\ temperature scaling) before using confidence to defer uncertain
cases to a human expert.

\begin{figure}[t]
\centering
\IfFileExists{figures/reliability_diagrams.png}{\includegraphics[width=0.98\linewidth]{reliability_diagrams.png}}{\fbox{\parbox{0.6\linewidth}{\centering\vspace{1cm}[reliability\_diagrams.png]\vspace{1cm}}}}
...
qualitative observation. While the models do place attention on visible lesions for
the disease classes, they also assign substantial saliency to leaf margins and to the
uniform background. We show representative single examples per class, so this is
illustrative rather than a quantitative attribution measurement; nonetheless it is
visually consistent with the background bias that
Noyan~\cite{noyan2022uncovering} and Barbedo~\cite{barbedo2018impact} documented,
suggesting that part of the models' apparent skill may derive from dataset artefacts
rather than from pathology. Pending a quantitative test (e.g.\ background ablation),
the headline accuracies are prudently read as an upper bound on real-world, in-field
performance.

\begin{figure}[t]
\centering
\IfFileExists{figures/gradcam_panel.png}{\includegraphics[width=0.95\linewidth]{gradcam_panel.png}}{\fbox{\parbox{0.6\linewidth}{\centering\vspace{1cm}[gradcam\_panel.png]\vspace{1cm}}}}
\caption{Grad-CAM overlays for one representative example per class. Warm regions are
the most influential for the predicted class. Attention spills onto leaf margins and
background, illustrating a reliance that is consistent with the PlantVillage
background bias (qualitative; not a quantitative attribution metric).}
\label{fig:gradcam}
\end{figure}

\section{Discussion}
Our results support a two-sided conclusion. On the one hand, a small CNN trained
from scratch on a laptop \emph{outperforms}---significantly and reproducibly---the
frozen-backbone transfer-learning baselines for this three-class problem, while using
$\paramratiomin$--$\paramratiomax\times$ fewer parameters. This should be read with
care: we freeze the pretrained backbones and train only a new head, the standard
low-compute transfer recipe, so the comparison is specifically \emph{from-scratch
end-to-end training versus frozen-feature transfer under an equal, modest compute
budget}. There is also an asymmetry worth stating plainly: because their backbones
are frozen, the baselines update only $\trainheadmin$--$\trainheadmax$ head parameters
during training, whereas the from-scratch CNN optimises all $\traincustom$ of its
weights. Our ``fewer parameters'' advantage therefore refers to \emph{total} model
size---what must be stored and executed at inference---not the number of weights
updated during training. Full fine-tuning of the backbones would very likely raise
their accuracy
and could narrow or reverse the gap, but it is also substantially more expensive and
data-hungry---precisely the cost we set out to avoid. The practical message stands:
for a narrow, well-defined diagnostic task on accessible hardware, a compact bespoke
model is not a compromise but a strong and far cheaper choice. On the other hand,
the Grad-CAM analysis is a cautionary result that no amount of additional accuracy on
PlantVillage can resolve: because the dataset's backgrounds are informative, high
within-dataset scores do not guarantee field robustness.
...
against frozen-backbone transfer-learning baselines for potato leaf disease
classification on commodity hardware. The compact custom model---more than an order
of magnitude smaller than ResNet-18 ($\paramratiomax\times$ fewer parameters)---%
significantly outperforms the frozen transfer baselines under an equal low-compute
budget, and Grad-CAM reveals a shared reliance
on dataset background that qualifies all reported accuracies. The work argues for
honest, interpretable and hardware-accessible plant-disease modelling over
leaderboard chasing.
